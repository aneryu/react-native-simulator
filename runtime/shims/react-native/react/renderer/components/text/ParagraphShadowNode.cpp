...
Content ParagraphShadowNode::getContentWithMeasuredAttachments(
  ...
  // Android replacement spans retain an inline host's explicit height even
  // when the Paragraph itself has an exact, shorter height. Constraining the
  // attachment by that paragraph height turned a 100dp inline View into 76dp
  // before text layout, so the host could never overflow/clamp like
  // TextInlineViewPlaceholderSpan. Keep the paragraph width as the wrapping
  // boundary but measure inline descendants with unbounded vertical space.
  localLayoutConstraints.maximumSize.height =
