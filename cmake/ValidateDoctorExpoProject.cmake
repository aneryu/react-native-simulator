...
string(JSON kind GET "${report}" project kind)
...
string(JSON metro_url GET "${report}" project metro url)
string(JSON metro_project_verified GET
  "${report}" project metro projectVerified)
string(JSON expo_detected GET "${report}" project expo detected)
string(JSON expo_router GET "${report}" project expo router)
string(JSON expo_sdk GET "${report}" project expo sdk)
string(JSON expo_app_key GET "${report}" project expo appRegistryKey)
string(JSON next_action GET "${report}" project nextAction)

if(NOT detected OR NOT compatible OR NOT preflight_passed OR
   NOT kind STREQUAL "expo" OR
   NOT declared STREQUAL "0.87.0" OR
   NOT platform STREQUAL "android" OR
   NOT profile STREQUAL "android-rn87" OR
   NOT app_key STREQUAL "main" OR
   NOT first_entry STREQUAL "node_modules/expo/AppEntry.js" OR
   NOT metro_required OR
   NOT expo_detected OR expo_router OR
   NOT expo_sdk STREQUAL "58.0.0-canary" OR
   NOT expo_app_key STREQUAL "main")
  message(FATAL_ERROR "Unexpected Expo doctor report: ${report}")
endif()

if(NOT metro_url MATCHES "node_modules/expo/AppEntry.bundle")
  message(FATAL_ERROR "Doctor Metro URL did not use Expo AppEntry: ${report}")
endif()
...
elseif(ready_to_launch OR
       ...
       NOT next_action MATCHES "npx expo start")
  message(FATAL_ERROR "Doctor omitted the Expo Metro action: ${report}")
endif()
